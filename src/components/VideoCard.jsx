...
const VideoCard = ({displayData,setDeleteVideopResponse,insideCategory}) => {  //object destructuring ({displayData})
  ...
  const handleShow = async () => {
    ...
    await saveHistoryAPI(videoDetails) //another videoDetails-object
  }

  // function for deleting videos (parent-view child-videocard)
  const deleteVideo = async (id) => {
    const result = removeVideoAPI(id) //in this step, vdo deleted from json file 
    setDeleteVideopResponse(result)
  }

  const videoDragStart = (e,videoId) =>{
    // console.log(`Video with id : ${videoId} has started dragging...`);
    e.dataTransfer.setData("id",videoId) //drag event inte "dataTransfer" - its a property that used to store the data that need to be trasffered when we drag the element  setData = to store that data
  }
